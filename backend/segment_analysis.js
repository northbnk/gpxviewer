...
try {
  ...
  console.log(
    "KM\tDist(m)\tGain(m)\tLoss(m)\tUpRate(%)\tDownRate(%)\tNetRate(%)\tPace(min/km)",
  );
  segments.forEach((seg, i) => {
    const paceStr =
      seg.pace_min_per_km == null ? "-" : seg.pace_min_per_km.toFixed(2);
    console.log(
      `${i + 1}\t${seg.dist_m.toFixed(0)}\t${seg.gain_m.toFixed(2)}\t${seg.loss_m.toFixed(2)}\t${seg.up_rate.toFixed(2)}\t${seg.down_rate.toFixed(2)}\t${seg.net_rate.toFixed(2)}\t${paceStr}`,
    );
  });
